...
class Reporter {
  constructor(config) {
    ...
    this.forceEndSkippedTests =
      this.config.reporterOptions.forceEndSkippedTests === undefined
        ? false
        : this.config.reporterOptions.forceEndSkippedTests;
  }
  ...
  suiteEnd(suite) {
    if (this.forceEndSkippedTests) {
      this.skippedTestItems.forEach((test, testId, map) => {
        this.testEnd(test);
        map.delete(testId);
      });
    }
  ...
  testStart(test) {
    ...
    if (this.forceEndSkippedTests) {
      this.skippedTestItems.set(test.id, test);
    }
  ...
  testEnd(test) {
    ...
    if (this.forceEndSkippedTests && test.status !== SKIPPED) {
      this.skippedTestItems.delete(test.id);
    }
